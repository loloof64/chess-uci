...
namespace
{
    ...
    struct EmbeddedNNUE
    {
        EmbeddedNNUE(const unsigned char *embeddedData,
                     ...
                     const unsigned int embeddedSize) : data(embeddedData),
                                                        end(embeddedEnd),
                                                        size(embeddedSize) {}
...
namespace Stockfish::Eval::NNUE
{
                                            ...
                                            const std::function<void(std::string_view)> &f) const
    {
        ...
        if (f)
        {
            ...
            f("NNUE evaluation using " + evalfilePath + " (" + std::to_string(size / (1024 * 1024)) + "MiB, (" + std::to_string(featureTransformer.TotalInputDimensions) + ", " + std::to_string(network[0].TransformedFeatureDimensions) + ", " + std::to_string(network[0].FC_0_OUTPUTS) + ", " + std::to_string(network[0].FC_1_OUTPUTS) + ", 1))");
        }
    }
