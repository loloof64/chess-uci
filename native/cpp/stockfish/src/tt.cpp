...
namespace Stockfish
{
        ...
        Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, uint8_t generation8)
    {
        ...
        if (b == BOUND_EXACT || uint16_t(k) != key16 || d - DEPTH_ENTRY_OFFSET + 2 * pv > depth8 - 4 || relative_age(generation8))
        {
    ...
    int TranspositionTable::hashfull(int maxAge) const
    {
        ...
        for (int i = 0; i < 1000; ++i)
            for (int j = 0; j < ClusterSize; ++j)
                cnt += table[i].entry[j].is_occupied() && table[i].entry[j].relative_age(generation8) <= maxAgeInternal;
    ...
    std::tuple<bool, TTData, TTWriter> TranspositionTable::probe(const Key key) const
    {
        ...
        for (int i = 1; i < ClusterSize; ++i)
            if (replace->depth8 - replace->relative_age(generation8) > tte[i].depth8 - tte[i].relative_age(generation8))
                replace = &tte[i];
